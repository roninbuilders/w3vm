...
import type { Chain, EIP6963Connector, Provider } from '@w3vm/core'
...
export class Web3Modal extends Web3ModalScaffold {
  ...
  public constructor(options: Web3ModalClientOptions) {
    console.log("Function: Web3Modal")
    ...
    const networkControllerClient: NetworkControllerClient = {
      ...
      async getApprovedCaipNetworksData(): Promise<any> {
        console.log("Function: getApprovedCaipNetworksData")
    ...
    const connectionControllerClient: ConnectionControllerClient = {
      connectWalletConnect: async onUri => {
        console.log("Function: connectWalletConnect")
        const connector = getW3.connectors().find(c => c.id === WALLET_CONNECT_CONNECTOR_ID)
        if (!connector) {
      ...
      connectExternal: async ({ id }) => {
        console.log("Function: connectExternal")
      ...
      checkInjectedInstalled(ids) {
        console.log("Function: checkInjectedInstalled")
        if (!window?.ethereum) {
          return false
        }
    ...
    const _chains = chains ?? [...connector?.options.optionalChains, ...connector?.options.chains]
    
    this.syncRequestedNetworks(_chains, chainImages)
  ...
  private syncRequestedNetworks(
    ...
    const requestedCaipNetworks = chains?.map(
      chain =>{
        ...
        const name = typeof chain === 'number' ? `Chain ${chain}` : chain.chainName
        return {
  ...
  private async syncAccount() {
    ...
    console.log("syncAccount", Boolean(chainId), chainId)
    this.resetAccount()
    if (address && chainId) {
  ...
  private async syncNetwork(chainImages?: Web3ModalClientOptions['chainImages']) {
    ...
    if (chainId) {
      ...
      console.log("chain id " ,NetworkImageIds[chainId])
      this.setCaipNetwork({
        id: caipChainId,
        name: "Chain "+ chainId,
        imageId: NetworkImageIds[chainId],
  ...
  private listenConnectors(connectors: Connector[]) {
