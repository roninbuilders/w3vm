...
import type { AppKitNetwork, CaipNetwork, CustomRpcUrlMap } from '@reown/appkit-common'
...
import { type ConnectorType, ConstantsUtil as CoreConstantsUtil, type Provider } from '@reown/appkit-controllers'
...
export class W3vmAdapter extends AdapterBlueprint {
	...
	private createConfig(
		...
		this.w3vmChains = configParams.networks
			.filter((cn) => cn.chainNamespace === CommonConstantsUtil.CHAIN.EVM)
			.map((caipNetwork) => {
				const customRpcs = caipNetwork.rpcUrls[caipNetwork.caipNetworkId]?.http
				const rpcUrls = customRpcs?.length
					? [...caipNetwork.rpcUrls.default.http, ...customRpcs]
					: [...caipNetwork.rpcUrls.default.http]
	...
	public formatUnits(params: AdapterBlueprint.FormatUnitsParams): AdapterBlueprint.FormatUnitsResult {
	...
	public async connect(params: AdapterBlueprint.ConnectParams): Promise<AdapterBlueprint.ConnectResult> {
	...
	public async getBalance(params: AdapterBlueprint.GetBalanceParams): Promise<AdapterBlueprint.GetBalanceResult> {
		...
		if (caipNetwork) {
			...
			this.balancePromises[caipAddress] = new Promise<AdapterBlueprint.GetBalanceResult>(async (resolve) => {
				try {
	...
	public override async switchNetwork(params: AdapterBlueprint.SwitchNetworkParams) {
		await Promise.all([switchChain({ chain: Number(params.caipNetwork.id) }), super.switchNetwork(params)])
	}

	public async getCapabilities(params: string) {
		const provider = w3vmStore.get('connectedWallet')?.provider as unknown as Awaited<
			ReturnType<(typeof UniversalProvider)['UniversalProvider']['init']>
		>
	...
	public async revokePermissions(params: AdapterBlueprint.RevokePermissionsParams): Promise<`0x${string}`> {
		const provider = w3vmStore.get('connectedWallet')?.provider
	...
	public override async setUniversalProvider(
		universalProvider: Awaited<ReturnType<(typeof UniversalProvider)['UniversalProvider']['init']>>,
	): Promise<void> {
