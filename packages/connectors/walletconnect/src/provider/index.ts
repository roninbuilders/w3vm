...
import { STORAGE_KEY, REQUIRED_METHODS, REQUIRED_EVENTS, RPC_URL, OPTIONAL_METHODS, OPTIONAL_EVENTS } from './constants'
...
export function buildNamespaces(params: NamespacesParams): {
	...
	const required: Namespace = {
		...
		rpcMap: {
			...(chains.length ? { [getEthereumChainId(chains)]: rpcMap[getEthereumChainId(chains)] as string } : {}),
		},
	}
	...
	const optional: Namespace = {
		...
		events: [...new Set(required.events.concat(optionalEvents?.length ? optionalEvents : OPTIONAL_EVENTS))],
		rpcMap,
	}
...
export class EthereumProvider implements IEthereumProvider {
	...
	public async connect(opts?: ConnectOps): Promise<void> {
		...
		try {
			const session = await new Promise<SessionTypes.Struct | undefined>(async (resolve, reject) => {
				const scopedProperties = opts?.scopedProperties ? { [this.namespace]: opts.scopedProperties } : undefined
	...
	public async authenticate(
		...
		try {
			const result = await new Promise<AuthTypes.AuthenticateResponseResult>(async (resolve, reject) => {
				await this.signer
					.authenticate(
						{
							...params,
	...
	protected setChainIds(chains: string[]) {
		...
		if (chainIds.length) {
			this.chainId = chainIds[0] as number
			this.events.emit('chainChanged', toHexChainId(this.chainId))
			this.persist()
		}
	}
	...
	protected parseAccountId(account: string): { chainId: string; address: string } {
		...
		return { chainId, address: address as string }
	}
	...
	protected async loadPersistedSession() {
		...
		try {
			...
			this.setChainIds(chainId ? [this.formatChainId(chainId)] : (namespace?.accounts as string[]))
			this.setAccounts(namespace?.accounts as string[])
		} catch (error) {
